...
export const generateHazards = async (apiKey: string, details: InstallationDetails): Promise<string> => {
  if (!apiKey) return "AI not active. Please provide an API key.";
  ...
  try {
    ...
    return "Error generating hazards. Please check your API key and the console, then try again. Or, enter hazards manually.";
  }
};


export const generateVisualWarnings = async (apiKey: string, hazardsText: string): Promise<string[]> => {
    if (!apiKey) return [];
    ...
    try {
        ...
        console.error("Error generating visual warnings:", error);
        return [];
    }
};

export const generateChecklist = async (apiKey: string, hazardsText: string): Promise<ChecklistItem[]> => {
  if (!apiKey) return [];
...
export const generateBriefing = async (
  ...
  if (!apiKey) return "AI not active. Briefing summary cannot be generated.";
  ...
  try {
    ...
    return "Error generating briefing summary. Please check your API key and the console.";
  }
};
